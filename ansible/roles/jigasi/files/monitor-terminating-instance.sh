...
QUEUE_NAME="jigasi-terminating"

#LOG RESULTS TO THIS PRE-CREATED SNS TOPIC
SNS_TOPIC_NAME="Jigasi-Termination-Log"

#UNIQUE LIFCE CYCLE HOOK NAME
LIFECYCLE_HOOK_NAME="jigasi-scale-down-gracefully"
...
#source the environment properties for better reporting
...
# create sqs jigasi-terminating
# create iam role jigasi-terminating-queue-publish
...
MESSAGE="Starting up termination monitoring for Jigasi on instance $EC2_INSTANCE_ID environment $ENVIRONMENT region $EC2_REGION"
...
do
    ...
    while [ -n "$msg" ] && [ ! "$msg" = "null" ] ;do
        ...
        if [ "${EVENT_TYPE}" = "autoscaling:TEST_NOTIFICATION" ]; then
            ...
            MESSAGE="Shutdown message matches for Jigasi TOKEN $ACTION_TOKEN HANDLE $RECEIPT_HANDLE_VALUE on instance $EC2_INSTANCE_ID for environment $ENVIRONMENT region $EC2_REGION"
            $DEBUG && aws sns publish --topic-arn=$SNS_TOPIC_ARN --message="$MESSAGE"
            ...
            # execute it in the background, anway aws will delete our machine
            HEART_BEAT_EXECUTION=$((HEART_BEAT_VAL-180))
            SLEEPTIME=0
            (while true; do
                MESSAGE="Waiting $HEART_BEAT_EXECUTION secs to check shutdown Jigasi for $SLEEPTIME on instance $EC2_INSTANCE_ID for environment $ENVIRONMENT region $EC2_REGION"
                $DEBUG && aws sns publish --topic-arn=$SNS_TOPIC_ARN --message="$MESSAGE"
                ...
                if [ $SLEEPTIME -gt $TERMINATION_DELAY_TIMEOUT ]; then
                    #LOG TIMEOUT TO SNS
                    MESSAGE="Timeout $SLEEPTIME > $TERMINATION_DELAY_TIMEOUT during graceful shutdown by Jigasi on instance $EC2_INSTANCE_ID for environment $ENVIRONMENT region $EC2_REGION"
                    $DEBUG && aws sns publish --topic-arn=$SNS_TOPIC_ARN --message="$MESSAGE"
                    # time to terminate
            ...
            if [[ $PID > 0 ]]; then
                SHUTDOWN_OUTPUT=$(/usr/share/jigasi/graceful_shutdown.sh 2>&1)
                SHUTDOWN_RESULT=$?
                if [ $SHUTDOWN_RESULT -eq 0 ]; then
                    #LOG SUCCESS TO SNS
                    MESSAGE="Successful graceful shutdown by Jigasi on instance $EC2_INSTANCE_ID for environment $ENVIRONMENT region $EC2_REGION"
                    $DEBUG && aws sns publish --topic-arn=$SNS_TOPIC_ARN --message="$MESSAGE"
                else
                    #LOG ERROR TO SNS
                    MESSAGE="Error $SHUTDOWN_RESULT during graceful shutting down Jigasi on instance $EC2_INSTANCE_ID for environment $ENVIRONMENT region $EC2_REGION: $SHUTDOWN_OUTPUT"
                    $DEBUG && aws sns publish --topic-arn=$SNS_TOPIC_ARN --message="$MESSAGE"

                fi
            else
                #LOG ERROR TO SNS
                MESSAGE="Error during graceful shutdown: No Jigasi found PID: $PID, Jigasi not running on instance $EC2_INSTANCE_ID for environment $ENVIRONMENT region $EC2_REGION: $SHUTDOWN_OUTPUT"
                $DEBUG && aws sns publish --topic-arn=$SNS_TOPIC_ARN --message="$MESSAGE"
            fi
...
MESSAGE="Ending termination monitoring for Jigasi on instance $EC2_INSTANCE_ID for environment $ENVIRONMENT region $EC2_REGION"
$DEBUG && aws sns publish --topic-arn=$SNS_TOPIC_ARN --message="$MESSAGE"

...
oci_vcn_md_read() {
  ...
  local s
  for s in $(# string
    grep -w subnetCidrBlock "$tmpfile" | cut -f 4 -d '"'
  ); do
    MD_SCIDRS+=(${s})
    MD_SPREFIXS+=(${s%/*})
    MD_SBITSS+=(${s#*/})
...
while [ $# -ge 1 ]; do
  ...
  -d) deconfig='t' ;;
  -e)
    if [ $# -lt 2 ]; then oci_vcn_err "secondary private IP address option requires <IP address> <VNIC OCID>"; fi
    SEC_ADDRS+=($1)
    shift
    SEC_VNICS+=($1)
    shift
    ;;
  -n)
    if [ "$os_maj_ver" = "ol-6" ] || [ "$os_maj_ver" = "centos-6" ]; then
  ...
  -h)
    ...
    ;;
  -q)
    if [ -n "$DEBUG" ]; then oci_vcn_err "cannot specify quiet with verbose"; fi
    QUIET='t'
    ;;
  -v)
    if [ -n "$QUIET" ]; then oci_vcn_err "cannot specify verbose with quiet"; fi
    DEBUG='t'
    ;;
